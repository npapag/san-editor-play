package converters
{
	...
	public class SanToGraphConverterFull
	{
		...
		private function san2graph(root:XML,parent:int,flow:int):void {	
			...
			switch (lname) {
				...
				case "d":
					...
					gxml.appendChild(getDecorator(root.@name,new_goal_cid,root.@type,root.@id));
					if (new_goal_cid>1) gxml.appendChild(getEdge(parent,new_goal_cid,root.@type,flow));
				break;
				case "a":
					//gxml.appendChild(getGoal(root.@name,new_goal_cid,sitName,condName));
					gxml.appendChild(getAction(root.@name,new_goal_cid,root.@type,root.@id));
					if (new_goal_cid>1) gxml.appendChild(getEdge(parent,new_goal_cid,root.@type,flow));
				break;
			}
			//gxml.appendChild(getGoal(root.@name,new_goal_cid,sitName,condName));
			...
			for each (var action:XML in root.a) {
				...
				if (String(action.@type).length==0) { // do not create empty node
					var cList:XMLList = (action as XML).children(); //processes any type of (g,c,d,..)
					for (var i:int = cList.length(); i>0; i--) { //reverse order
						trace("reverse order " + i);
						san2graph(cList[i-1],new_goal_cid,newflow);
					} //works : the same with
					/*
					...
					*/
					...
					gxml.appendChild(getAction(action.@name,new_goal_cid,action.@type,action.@id));
			...
			if (condName.length > 0) {
				...
				gxml.appendChild(getCondition(condName,new_cond_cid,condID));
			...
			if (sitName.length > 0) {
				...
				gxml.appendChild(getSituation(sitName,new_sit_cid,sitID));
			...
			for each (var decor:XML in root.d) {
				...
				var oldcid:int = new_goal_cid;
				this.cid++; new_goal_cid = this.cid;										
				gxml.appendChild(getDecorator(decor.@name,new_goal_cid,decor.@type,decor.@id));
				gxml.appendChild(getEdge(oldcid,new_goal_cid,decor.@type,flow));
				
				var cList:XMLList = (decor as XML).children(); //processes any type of (g,c,d,..)
				for (var i:int = cList.length(); i>0; i--) { //reverse order
					trace("reverse order " + i);
					san2graph(cList[i-1],new_goal_cid,newflow);
				} //works : the same with
		...
		private function getGoal(name:String,id:int,sit_name:String,con_name:String,node_id:String):XML {			
			//return new XML(ret);
			var x:XML =  
				<Node id={id} name={name} desc='_desc' type='Goal' node_id={node_id}
		...
		private function getCondition(name:String,id:int,node_id:String):XML {			
			return <Node id={id} name={name} node_id={node_id}
				type='Condition' nodeColor='0xff0099' nodeSize='25' />;
		}
		
		private function getSituation(name:String,id:int,node_id:String):XML {			
			return <Node id={id} name={name}  node_id={node_id}
				type='Situation' nodeColor='0xff0099' nodeSize='25' />
		}
		
		private function getAction(name:String,id:int,type:String,node_id:String):XML {			
			return <Node id={id} name={name} desc={type+"_desc"} node_id={node_id}
				 subtype={type}
				type='action' nodeColor='0xff0099' nodeSize='25' />;
		}
		
		private function getDecorator(name:String,id:int,type:String,node_id:String):XML {			
			return <Node id={id} name={name} desc={type+"_desc"} node_id={node_id}
				subtype={type}
